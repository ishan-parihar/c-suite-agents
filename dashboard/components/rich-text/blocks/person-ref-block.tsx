...
export const personRefBlock = createReactBlockSpec(
  {
    type: "person-ref",
    propSchema: {
      id: { default: "" },
  ...
  },
  {
    render: ({ block }) => {
      ...
      const title = (block.props.title as string) || "Untitled Person";
      const status = block.props.status as string;
      const colors = ENTITY_REF_COLORS.person;
      const route = ENTITY_REF_ROUTES.person;

...
export const taskRefBlock = createReactBlockSpec(
  {
    type: "task-ref",
    propSchema: {
      id: { default: "" },
  ...
  },
  {
    render: ({ block }) => {
      ...
      const title = (block.props.title as string) || "Untitled Task";
      const status = block.props.status as string;
      const colors = ENTITY_REF_COLORS.task;
      const route = ENTITY_REF_ROUTES.task;
      ...
      return (
        <a
          ...
          title={`Open ${ENTITY_REF_LABELS.task}: ${title}`}
          onClick={(e) => e.preventDefault()}
        >
          <span className={`flex h-5 w-5 items-center justify-center rounded-full ${colors.bg}`}>
            <CheckSquare className={`h-3.5 w-3.5 ${colors.icon}`} />
          </span>

...
export const goalRefBlock = createReactBlockSpec(
  {
    type: "goal-ref",
    propSchema: {
      id: { default: "" },
  ...
  },
  {
    render: ({ block }) => {
      ...
      const title = (block.props.title as string) || "Untitled Goal";
      const status = block.props.status as string;
      const colors = ENTITY_REF_COLORS.goal;
      const route = ENTITY_REF_ROUTES.goal;
